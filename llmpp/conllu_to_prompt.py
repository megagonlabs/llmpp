...
CONLLU_TOKEN_SKIP_PATTERN = re.compile(
    r"^(([1-9][0-9]*[\-.][1-9][0-9]*)\t|# (sent_id =|text_en =|translit =|source =|generator =|udpipe_model =|note =|auto =|ToDoOrigText =|ToDoOrigtext =|spelling =|genre =|orig_file_sentence|duplicate:) ).+$|^#$|^0(.+)$|^# Tectogrammatical annotation available(.+)$"
)
