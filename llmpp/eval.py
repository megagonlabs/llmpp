    ...
    for line_index, messages in enumerate(completion_results, 1):
        ...
        if f_report:
            if gold_text == content_text:
                print("=", gold_text.strip() or input_text, file=f_report)
            else:
        ...
        if f_conllu:
            print(f"# text = {gold_text.strip() or input_text}", file=f_conllu)
...
def parse_records(content: str, index_field: int, stop_on_error: bool = False) -> list[dict]:
    ...
    for r in rows:
        try:
            ...
            if field_num == 3:
                records.append({"index": int(r[0]), "form": " ", "head": 1 if r[1] == "_" else int(r[1]), "deprel": r[2]})
            elif field_num == 4:
                records.append({"index": int(r[0]), "form": " ", "upos": r[1], "head": 1 if r[2] == "_" else int(r[2]), "deprel": r[3]})
